...
export default function CardInfo({ data, legalities, rarity }) {
    return (
        <div
            ...
            lg:min-w-[initial] lg:max-w-[initial] lg:m-0 lg:h-fit lg:my-auto lg:flex-1`}>
            {data.map((dataSection, index) => (
                <div
                    ...
                    <div className="flex gap-2 items-center px-2 text-lg lg:pl-4">
                        <div>{dataSection.name}</div>
                        {dataSection.manaCost && (
                            <SymbolContainer symbols={dataSection.manaCost} />
                        )}
                    </div>
                    <div className="pt-2 px-2 lg:pl-4">{dataSection.typeLine}</div>
                    {dataSection.effect && (
                        <div className="pt-2 px-2 all-inline-block lg:pl-4">
                            <CardEffect data={dataSection.effect} />
                        </div>
                    )}
                    {dataSection.flavorText && (
                        <div className="italic pt-2 px-2 lg:pl-4">
                            {dataSection.flavorText}
                        </div>
                    )}
                    {dataSection.power && (
                        <div className="pt-2 px-2 lg:pl-4">
